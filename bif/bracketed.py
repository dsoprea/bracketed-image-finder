...
def _get_exposure(filepath):
	exposure_value = _read_tag(filepath, _EXPOSURE_TAG_ID)
...
def _is_float_equal(a, b):
	"""Account for epsilon."""

	return abs(a - b) < 0.0001

def _find_bracketing_sequence_at_tail(history):
	len_ = len(history)
	for expected_size in _BRACKETING_SEQUENCE_SIZES:
		...
		if _is_float_equal(sequence_exposure_delta, 0.0) is True:
			continue

		# Check if the polarity oscillation matches a known pattern.
		sequences = \
			_get_accepted_exposure_sequence(
				last_n_entries,
				sequence_exposure_delta, 
				expected_size)
		...
		for accepted_sequence in sequences:
			if _sequence_matches(current_sequence, accepted_sequence) is True:
				return \
					_BRACKET_INFO(
						size=expected_size, 
...
def _sequence_matches(sequence1, sequence2):
	len1 = len(sequence1)
	len2 = len(sequence2)
	...
	for i, x in enumerate(sequence1):
		if _is_float_equal(x, sequence2[i]) is False:
			return False

	return True


def _get_accepted_exposure_sequence(last_n_entries, exposure_delta, n):
	sequences = []
...
def find_bracketed_images(root_path):
	_HISTORY = []
	for path, folders, files in os.walk(root_path):
		...
		for filename in files:
			...
			try:
				exposure_mode_value = _read_tag(filepath, _EXPOSURE_MODE_TAG_ID)
				...
				exposure_value = _get_exposure(filepath)

				timestamp_raw = _read_tag(filepath, _DATETIME_TAG_ID)
			...
			bi = _find_bracketing_sequence_at_tail(_HISTORY)
			if bi is None:
				continue

			yield bi.exposure_delta, _HISTORY[-bi.size:]

def _bracketinfo_matches(bi1, bi2):
	return \
		bi1.size == bi2.size and \
		bi1.exposure_delta == bi2.exposure_delta and \
		_sequence_matches(bi1.sequence, bi2.sequence) is True
